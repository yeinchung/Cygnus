...
# Define the column names and updated proportions of 1's for each column
columns <- c('A', 'B', 'C', 'D', 'E')
proportions <- c(0.5, 0, 0.3, 0.2, 0)  # Increased proportions for A, D, and E
...
# Function to create a binary vector with a fixed number of 1's and 0's
create_column <- function(proportion, rows) {
  num_ones <- floor(rows * proportion)
  column_data <- c(rep(1, num_ones), rep(0, rows - num_ones))
  return(sample(column_data))  # Randomly shuffle the column
}

# Create the binary columns and fill the data frame
for (i in 1:length(columns)) {
  binary_exp_matrix[, i] <- create_column(proportions[i], rows)
}

# Now, we want to make it more likely for columns A, D, and E to have 1s at the same time
# We achieve this by adjusting positions probabilistically

# Adjust A, D, E to have correlated 1s
shift_factor <-0.8 # Control the correlation between A, D, and E (higher is stronger correlation)

# Track the indices where A is 1
colnames(binary_exp_matrix) <- c("A", "B","C","D","E")
A_indices <- which(binary_exp_matrix$A == 1)

# Create a random shuffle for the D and E columns to increase correlation with A
for (idx in A_indices) {
  if (TRUE) { #runif(1) < shift_factor
    # If A is 1 and the random factor is smaller than shift_factor,
    # we make sure D and E also have 1 in this row
    binary_exp_matrix[idx, "B"] <- 1
    binary_exp_matrix[idx, "E"] <- 1
  }
}

colSums(binary_exp_matrix)
proportions <- c(0.5, 0.4, 0.3, 0.2, 0.1)
# Make sure the total number of 1s in each column remains the same after adjustment
# Rebalance the columns by keeping the proportions intact
for (col in c("B", "E")) {
